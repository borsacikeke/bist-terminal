...
AYI_SINYALLER = ["MACD Sat Kesisimi","MACD Negatif","Fiyat EMA20 Altinda","EMA20 < EMA50",
                 ...
                 "3 Siyah Karga","Kara Bulut Ortusu","RSI Asiri Alim","BB Ust Bant"]
    ...
    for g, grup in enumerate(gruplar):
        ...
        for hisse in grup:
            try:
                ...
                # SMA200 sinyalleri
                if not (sma200 != sma200): # NaN kontrolu
                ...
                # Strateji tespitleri
                dip_avcisi = (25 <= rsi <= 42 and kapanis > sma200 * 0.97 and not (sma200 != sma200))
                patlama_oncesi = (bb_width < bb_width_ort * 0.7 and not (bb_width_ort != bb_width_ort))
                roket_rampa = (kapanis >= bb_ust * 0.98 and hacim_orani >= 2 and rsi > 55)
                destek_kalkani = (abs(kapanis - sma200) / sma200 < 0.04 and rsi < 48 and not (sma200 != sma200))
                akilli_para = (hacim_orani >= 2 and v["Close"].iloc[-1] > v["Open"].iloc[-1] and kapanis > ema20)
                trend_sorfcusu = (ema20 > ema50 and macd_val > macd_sig_val and 48 <= rsi <= 72)
