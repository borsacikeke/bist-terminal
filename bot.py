...
async def buton(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "altin_grafik":
        ...
        await query.edit_message_text(f"🏆 *{len(altin_hisseler)} Altın Sinyal* grafiği gönderiliyor...", parse_mode="Markdown")

        for ad in altin_hisseler:
            grafik_yolu = f"C:/bist_terminal/grafikler/{ad}.png"
            if os.path.exists(grafik_yolu):
                sinyaller = ", ".join(veri["hisseler"][ad]["sinyaller"][:4])
